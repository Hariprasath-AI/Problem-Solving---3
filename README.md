...
## :nine: series_sum = ((((last possible interval count ** 2) - last possible interval count) / 2) + last possible interval count) - (((((second number interval count - 1) ** 2) - (second number interval count - 1)) / 2) + (second number interval count - 1))
### NOTE :pencil2: : Substituting 'second number interval count' as 'min' and 'last possible interval count as 'max' in the formula from approach - 2 "((((max ** 2) - max) / 2) + max) - (((((min - 1) ** 2) - (min - 1)) / 2) + (min - 1))" which is series sum.
